...
function updateMousePosition(eX, eY) {
    ...
    pointer.y = -((eY - containerEl.offsetTop) / containerEl.offsetHeight)
 * 2 + 1;
}
...
function createOrbitControls() {
    ...
    controls.addEventListener("start", () => {
        ...
        const fenetre = document.querySelector("info-window")
        // <button onclick="myFunction()">Click me</button>
        // ICI IL FAUT METTRE LA FENETRE D'INFOS SI CLIC
        isHoverable = false;
        pointer = new THREE.Vector2(-1, -1);
...
function updateMap(uv = {x: 0, y: 0}) {
    ...
    for (let i = 0; i < svgCountries.length; i++) {
        ...
        if (
            ...
            if (isHovering) {
                //METTRE LA FENETRE ICI SI HOVERABLE
